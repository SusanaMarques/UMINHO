...
void bezierPatch(string pFile, int tessellation, FILE *f) {
    ...
    // Stores "Normals" we calculate
    Vec3f *Normals = new Vec3f[(tessellation + 1) * (tessellation + 1)];

    // Stores "texture coordinates" we calculate
    Vec2f *Textures = new Vec2f[(tessellation + 1) * (tessellation + 1)];
    ...
    for (int i = 0; i < nrPatches; i++) {
        ...
        for (int j = 0, z = 0; j <= tessellation; j++) {
            ...
            for (int m = 0; m <= tessellation; m++, z++) {
                ...
                Vec3f dU = dUBezier(controlPoints, m / (float)tessellation, j / (float)tessellation);
                Vec3f dV = dVBezier(controlPoints, m / (float)tessellation, j / (float)tessellation);
                Normals[z] = dU.crossProduct(dV).normalize();
                Textures[z].x = (double) m / (double) tessellation;
                Textures[z].y = (double) j / (double) tessellation;
            }
        ...
        for (int j = 0, z = 0; j < tessellation; j++) {
            ...
            for (int m = 0; m < tessellation; m++, z++) {
                ...
                fprintf(f, "%f %f %f\n", Normals[vc].x, Normals[vc].y, Normals[vc].z);
                fprintf(f, "%f %f\n", Textures[vc].x, Textures[vc].y);

                fprintf(f, "%f %f %f\n", BezierPoints[vb].x, BezierPoints[vb].y, BezierPoints[vb].z);
                fprintf(f, "%f %f %f\n", Normals[vb].x, Normals[vb].y, Normals[vb].z);
                fprintf(f, "%f %f\n", Textures[vb].x, Textures[vb].y);

                fprintf(f, "%f %f %f\n", BezierPoints[va].x, BezierPoints[va].y, BezierPoints[va].z);
                fprintf(f, "%f %f %f\n", Normals[va].x, Normals[va].y, Normals[va].z);
                fprintf(f, "%f %f\n", Textures[va].x, Textures[va].y);

                fprintf(f, "%f %f %f\n", BezierPoints[vd].x, BezierPoints[vd].y, BezierPoints[vd].z);
                fprintf(f, "%f %f %f\n", Normals[vd].x, Normals[vd].y, Normals[vd].z);
                fprintf(f, "%f %f\n", Textures[vd].x, Textures[vd].y);
